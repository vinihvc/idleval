...
describe("offline-earnings", () => {
  ...
  it("returns empty results when elapsed time is zero", () => {
    const now = 1_000_000;
    const result = computeOfflineEarnings(now, now, initialData, D(1));
  ...
  it("skips locked and non-automated factories", () => {
    ...
    const result = computeOfflineEarnings(elapsedMs, 0, factories, D(1));
  ...
  it("calculates secondsRemaining for partial and complete cycles", () => {
    ...
    const complete = computeOfflineEarnings(
      completeElapsedMs,
      0,
      factories,
      D(1)
  ...
  it("returns results in FACTORY_TYPES order", () => {
    ...
    const result = computeOfflineEarnings(120_000, 0, factories, D(1));
